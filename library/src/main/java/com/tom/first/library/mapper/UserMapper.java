...
public class UserMapper {
	...
	public User toUser(UserRequest request) {
		...
		return User.builder().username(request.username()).email(request.email()).password(request.password())
				.age(request.age()).build();
	}

	public UserResponse fromUser(User user) {
		return new UserResponse(user.getUsername(), user.getEmail(), user.getAge(), user.getCreatedDate(),
